...
export default function CommunitiesPage() {
  ...
  const fetchSuggested = async () => {
    ...
    if (res.ok) {
      const data = await res.json();
      const names = (data.items ?? []).slice(0, 5).map((item: { name: string }) => item.name);
      setSuggested(names);
    }
  };
  ...
  const fetchPeopleSuggestions = async () => {
    ...
    if (res.ok) {
      const data = await res.json();
      const items = (data.items ?? []).map((item: { user: { id: string; name: string | null } }) => ({
        id: item.user.id,
        name: item.user.name,
  ...
  const fetchTrendingTopics = async () => {
    ...
    if (res.ok) {
      const data = await res.json();
      const counts = (data.items ?? []).reduce((acc: Record<string, number>, post: { type: string }) => {
      ...
      const mapped = (Object.entries(counts) as Array<[string, number]>)
        .map(([label, count]) => ({
          label: label.toLowerCase() === "help" ? "Help" :
            ...
            label.toLowerCase() === "question" ? "Jobs" :
            label.toLowerCase() === "general" ? "Events" : label,
          count,
        }))
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter((post) => {
      ...
      const matchesLocation = location
        ? (post.locationContext ?? "").toLowerCase().includes(location.toLowerCase()) ||
          location.toLowerCase().includes((post.locationContext ?? "").toLowerCase())
        : true;
  ...
  if (
    redirected ||
    status === "loading" ||
    (status === "authenticated" && !checkedOnboarding)
  ) {
    return (
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 md:px-6">
        ...
        <aside className="hidden lg:flex lg:flex-col lg:gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold">
                  {session?.user?.name ?? "Homely member"}
                </p>
          ...
          <div className="rounded-2xl border bg-white p-4">
            ...
            <div className="mt-3 space-y-2 text-sm">
              ...
              {suggested.map((name) => (
                <div key={name} className="rounded-lg border bg-slate-50 px-3 py-2">
                  {name}
                </div>
              ))}
            </div>
            ...
            <article key={post.id} className="rounded-2xl border bg-white p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-500">
                <span className="flex items-center gap-1">
                  <HiOutlineChatBubbleLeftEllipsis /> {post._count?.comments ?? 0}
                </span>
                <span className="flex items-center gap-1">
                  <HiOutlineHeart /> {post._count?.reactions ?? 0}
                </span>
                <button className="rounded-full border px-3 py-1">Comment</button>
      ...
      <button
        className="fixed bottom-6 right-6 h-14 w-14 rounded-full bg-blue-600 text-2xl text-white shadow-lg"
